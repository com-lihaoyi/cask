 ...
 *
 * Configuration is process-scoped and loaded once lazily when first accessed.
 * The configuration is immutable after loading - changes to config files require
 * an application restart.
 *
 * Auto-loads configuration at startup from (in priority order):
 * - System properties
 * - application.conf (HOCON format)
 * - application.json (JSON format)
 * - application.properties (Java properties)
 * - application-{CASK_ENV}.conf (environment-specific overrides)
